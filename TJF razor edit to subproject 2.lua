...
PromptForFilename = true                 -- If true, script will ask user to name the subproject being created
...
UserVideoTrackName = "PIC CUT"           -- optional custom video track name for user - NEEDS QUOTATION MARKS
 ...
 function GetRppProx()
      ...
      local fullrecordpath = reaper.GetProjectPath("" )
      
      local _, recordpathshort = reaper.GetSetProjectInfo_String( 0, "RECORD_PATH", "", false )
      
      local sessionpath = string.gsub(fullrecordpath, recordpathshort, "") .. name .. "-PROX"
