...
--[[      FUNCTION NO LONGER USED  -- exploiting auto crossfade instead.. retaining for later reference
...
function ProccessFades(items, starttime, endtime) -- items should be a table
    ...
    for j = 1, #items do
      ...
      then
            ...
            if    itemstart >= starttime and itemstart <= endtime and itemend > endtime --and j==1  -- if the item starts within bounds but ends outside of them  *** commented section allows multiple files on same lane to adjust to razor edit
            then
            ...
            elseif itemstart < starttime and itemend > starttime and itemend < endtime --and j == (#items)  -- if the item starts outside of bounds, but ends within them  *** commented section allows multiple files on same lane to adjust to razor edit
            then
...
function Main()
      ...
      then
            ...
            items = {}                                -- Create 2 table.  items = items that fall within the time selection
            selitems = {}                             --                  selitems = items that fall within the time seleciton that are also selected
