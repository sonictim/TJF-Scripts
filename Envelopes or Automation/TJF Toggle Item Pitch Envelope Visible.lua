--[[
@description TJF Toggle Item Pitch Envelope Visible
@version 2.3
@author Tim Farrell
@link http://github.com/sonictim/TJF-Scripts/
@date 2020 04 26
@about
  # TJF Toggle Item Pitch Envelope Visible
  Will Toggle Pitch Envelope Visibility for all selected items.
...
function ToggleTakeEnvelope(kind) -- Toggles the take envelope visibility Volume, Pan, or Pitch
      ...
      for i=1, #items do
            ...
            local     Env = string.match(str, "<PITCHENV.->")      --find the volume envelope
            
            if not    Env 
            then      Env = CreateNewEnv("<PITCHENV") 
            end
      ...
      if #items == 1 then 
          reaper.SetCursorContext( 2, reaper.GetTakeEnvelopeByName(reaper.GetActiveTake(items[1]), "Pitch" )) -- selects envelope
