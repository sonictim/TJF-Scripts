--[[
@description TJF Toggle Pan Envelope Visible for Track or Items
@version 2.3
@author Tim Farrell
@link http://github.com/sonictim/TJF-Scripts/
@date 2020 04 26
@about
  # TJF Toggle Pan Envelope Visible for Track or Items
  Will Toggle Pan Envelope Visibility for all selected Items, or Tracks if no items selected.
...
@changelog
  ...
  v2.0 logic rework
  v2.1 removed all SWS functions (re-coded from scratch)
  v2.2 accounts for trim volume on folders
  v2.3 bugfix for take envelopes
--]]
...
function ToggleTakeEnvelope(kind) -- Toggles the take envelope visibility Volume, Pan, or Pitch
      ...
      for i=1, #items do
            ...
            local     VolEnv = string.match(str, "<PANENV.->")      --find the volume envelope
            
            if not    VolEnv 
            then      VolEnv = CreateNewEnv("<PANENV") 
            end
      ...
      if #items == 1 then 
          reaper.SetCursorContext( 2, reaper.GetTakeEnvelopeByName(reaper.GetActiveTake(items[1]), "Pan" )) -- selects envelope
...
function Main()
    ...
    if      #items > 0
    then
            ToggleTakeEnvelope("Pan")
          
    elseif  #tracks > 0 
    then    toggletrackvisible("Pan")
