              ...
              for i=1, itemcount do  --For each item do
                      ...
                      for j=0, takeEnvCount-1 do  -- for each take envelope for this item do
                                    ...
                                    if OtherEnv == NamedEnv then
                                         ...
                                         reaper.BR_EnvSetProperties(brTakeEnv, true, visible, armed, inLane, laneHeight, defaultShape, faderScaling)
                                         reaper.BR_EnvFree(brTakeEnv, true )
                        end--for
...
if not trackcount then return end
    ...
    if param == "Volume (Pre-FX)" then reaper.Main_OnCommand(40408, 0) end --toggle track volume envelope visible 
...
function Main()
    ...
    if  itemcount > 0 then
        ...
        toggletrackvisible("Volume (Pre-FX)")
